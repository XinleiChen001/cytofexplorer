...
cluster_significant_plot<-function(cluster_stat,
  ...
  cluster_count_stat$cluster_num<-sub("cluster","",row.names(cluster_count_stat))
  ...
  cluster_percent_stat$cluster_num<-sub("cluster","",row.names(cluster_percent_stat))
...
draw_abundance_boxplot<-function(cluster_stat,cluster_id=NA
){
  ...
  heatmap_ctrl    <- as.character(cluster_stat[["metadata"]]$heatmap_ctrl)
   ...
   stat.paired        <- cluster_stat[["metadata"]]$stat.paired
   stat.method        <- as.character(cluster_stat[["metadata"]]$stat.method)
  # cluster_cell_count<- cluster_stat[["cluster_cell_count"]]
  groups            <-cluster_stat[["groups"]]
  # all_markers       <-cluster_stat[["all_markers"]]
  major_cond<-as.character(cluster_stat[["metadata"]]$major_cond)
  # stat.method    <- cluster_stat[["metadata"]]$stat.method

  cluster_count_summary<-cluster_stat[["cluster_count_summary"]]
  cluster_percent_summary<-cluster_stat[["cluster_percent_summary"]]

  if(is.na(cluster_id[1])){
  ...
  cluster_boxplot<-function(cluster_name){

    ggplot(cluster_boxplot_data,aes_string(x=major_cond,y=cluster_name,color=major_cond))+
      ...
      scale_colour_manual(values=brewer_color_sets(length(unique(col_color_index))))+
      labs(y="Percentage %")+
  ...
  cluster_boxplot_list<-lapply(cluster_names,cluster_boxplot)
  length(cluster_boxplot_list)

  # colnum=ceiling(sqrt(length(cluster_boxplot_list)))
  # rownum=ceiling(length(cluster_boxplot_list)/colnum)
  pdf(file=paste0("Cluster Abundance Boxplot.pdf"), width=boxplot_width, height=boxplot_height)
  multiplot(plotlist=cluster_boxplot_list,cols = boxplot_ncol)
  dev.off()
  cat(paste0("Fig output finished\n"))
}
...
draw_abundance_volcano<-function(cluster_stat,
  ...
  FDR_table<-ggtable(FDR_table_data)
  ...
  FDR_pValue_plot<-ggplot(FDR_pValue)+
    ...
    coord_flip()+
    labs(subtitle=paste0("FDRs were calculated based on ",p.adjust.method," adjust method"))+
    mytheme+
